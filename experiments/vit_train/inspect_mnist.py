...
def run(weights: Path = "./"):
    ...
    ax.scatter(zero_proj[:,0], zero_proj[:,1], color='cyan', label='Actual Zero')
    ax.scatter(one_proj[:,0], one_proj[:,1], color='maroon', label='Actual One')
    ax.scatter(midpoint_proj[:,0], midpoint_proj[:,1], color='black', label="ACtual Midpoint")
    plt.legend()
