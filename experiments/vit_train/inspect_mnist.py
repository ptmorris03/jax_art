...
def run(weights: Path = "./"):
    ...
    fig = plt.figure()
    ax = fig.add_subplot(1,1,1)
    ax.scatter(ball_proj[dce_one,0], ball_proj[dce_one,1], color='red', s=.001, label="one")
    ax.scatter(ball_proj[dce_zero,0], ball_proj[dce_zero,1], color='blue', s=.001, label="zero")
    ax.scatter(ball_proj[np.logical_and(ball_other, dce_one),0], ball_proj[np.logical_and(ball_other, dce_one),1], color='green', s=10, label="cls_other one")
    ax.scatter(ball_proj[np.logical_and(ball_other, dce_zero),0], ball_proj[np.logical_and(ball_other, dce_zero),1], color='yellow', s=10, label="cls_other zero")
    ax.scatter(zero_proj[:,0], zero_proj[:,1], color='blue', s=100, label='Actual Zero')
    ax.scatter(one_proj[:,0], one_proj[:,1], color='red', s=100, label='Actual One')
    ax.scatter(midpoint_proj[:,0], midpoint_proj[:,1], color='black', s=100, label="Actual Midpoint")
    plt.legend()
